...
int main(int argc, char **argv)
{
	...
	assert(1); // TBD: Fill in ucontext size above. Hint: use sizeof().
	...
	printf("The number of bytes pushed onto the stack between err and when the stack was saved to mycontext was 0x%x\n", 
			(unsigned int)((void *)&mycontext - (void *)&err));
...
{
	assert(1); /* Write code for this function */
	ucontext_t mycontext;
